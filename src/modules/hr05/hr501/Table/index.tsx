...
interface Column {
  id: keyof EvaluationRoundData | 'actions'; // เปลี่ยนเป็น keyof EvaluationRoundData
  label: string;
  minWidth?: number;
...
type DataTableProps = {
  ...
  setTableData: React.Dispatch<React.SetStateAction<EvaluationRoundData[]>>; // เปลี่ยนเป็น EvaluationRoundData[]
  onView: (data: EvaluationRoundData) => void;
  onEdit: (data: EvaluationRoundData) => void;
...
const DataTable = ({ data, setTableData, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof EvaluationRoundData>('evaluation_round_id'); // เปลี่ยนเป็น evaluation_round_id
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'evaluation_round_id', label: 'รหัสรอบการประเมิน', minWidth: 80, sortable: true },
      { id: 'year', label: 'ปี', minWidth: 50, sortable: true },
      { id: 'round_name', label: 'ชื่อรอบการประเมิน', minWidth: 200, sortable: true },
  ...
  const handleRequestSort = (
    ...
    property: keyof EvaluationRoundData, // เปลี่ยนเป็น keyof EvaluationRoundData
  ) => {
        ...
        <Table stickyHeader aria-label="sticky table">
                ...
                >
                  {column.sortable ? (
                    <TableSortLabel
                      ...
                      onClick={(event) => handleRequestSort(event, column.id as keyof EvaluationRoundData)} // เปลี่ยนเป็น keyof EvaluationRoundData
                    >
                      {column.label}
                      {orderBy === column.id ? (
          ...
          <TableBody>
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  key={row.evaluation_round_id} // เปลี่ยน key เป็น evaluation_round_id
                  data={row}
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.evaluation_round_id)} // เปลี่ยน onDelete เป็น evaluation_round_id
                />
              ))}
          </TableBody>
        </Table>
