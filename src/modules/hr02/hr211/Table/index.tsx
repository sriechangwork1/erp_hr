...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof AcademicExpertiseData>('expertise_id');
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'expertise_id', label: 'รหัสความเชี่ยวชาญ', minWidth: 80, sortable: true },
      { id: 'staff_id', label: 'รหัสบุคลากร', minWidth: 100, sortable: true },
      { id: 'expertise_name', label: 'ชื่อความเชี่ยวชาญ', minWidth: 180, sortable: true },
      { id: 'isced_code', label: 'รหัส ISCED', minWidth: 100, sortable: true },
      { id: 'subject_code', label: 'รหัสหมวดวิชา', minWidth: 100, sortable: true },
      { id: 'teaching_level', label: 'ระดับที่สอน', minWidth: 100, sortable: true },
      { 
        id: 'create_at', 
        label: 'วันที่สร้าง', 
        minWidth: 120, 
            ...
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  ...
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.expertise_id)}
