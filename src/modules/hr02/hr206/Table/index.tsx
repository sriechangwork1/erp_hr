...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof Data>('work_history_id');
  const [searchQuery, setSearchQuery] = React.useState<string>('');

  const intl = useIntl();

  const labeltext = React.useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr02.06' }); // Assuming new ID
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      ...
      { id: 'appointment_number', label: 'เลขที่คำสั่ง', minWidth: 120, sortable: true },
      { id: 'appointment_date', label: 'วันที่ออกคำสั่ง', minWidth: 120, sortable: true },
      { id: 'work_status', label: 'สถานภาพ', minWidth: 120, sortable: true },
      { id: 'start_work_date', label: 'วันที่เริ่มงาน', minWidth: 120, sortable: true },
      { id: 'retirement_date', label: 'วันที่เกษียณ/ลาออก', minWidth: 150, sortable: true },
            ...
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  ...
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.work_history_id)}
                />
              ))}
          </TableBody>
        </Table>
