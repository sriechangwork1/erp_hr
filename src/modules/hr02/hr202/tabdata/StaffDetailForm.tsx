...
// --- Define data type for each row in the table (from Staff model) ---
...
// --- Options for Dropdown (move to the same file or import from somewhere else) ---
...
interface StaffDetailFormProps {
  formData: StaffData;
  setFormData: React.Dispatch<React.SetStateAction<StaffData>>;
  handleSave: () => void;
  handleCancel: () => void;
}
...
const StaffDetailForm: React.FC<StaffDetailFormProps> = ({
  ...
  const handleInputChange = (
    ...
    if (name) {
      const numberFields = [
        'prefixname_id', 'faculty_id', 'department_id', 'program_id',
        'staff_status', 'staff_type_id', 'position_type_id', 'job_title_id',
        'work_line_id', 'position_level_id', 'academic_position_id',
        'support_position_id', 'admin_position_id', 'officer_id',
        'weight', 'height', 'staff_id'
      ];

      let newValue: string | number | undefined = value as string;
  ...
  return (
    <Box sx={{ p: 3, border: '1px solid #e0e0e0', borderRadius: '8px', mt: 3 }}>
      ...
      <Grid container spacing={2}>
        {/* Grid item for the profile picture */}
        <Grid item xs={12} sm={6}>
          <Box sx={{ display: 'flex', alignItems: 'center', mt: 2, mb: 1 }}>
            ...
            >
              {!formData.profile_picture && <MdAccountCircle style={{fontSize: '4rem',color: '#757575' }} />}
            </Avatar>
            <TextField
              label="รหัสบุคลากร"
              variant="outlined"
