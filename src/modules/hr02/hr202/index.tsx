...
import { Autocomplete, TextField, Grid, Button, Box, MenuItem } from '@mui/material'; // นำเข้าทั้งหมดที่จำเป็น
...
const StaffDetailForm: React.FC<StaffDetailFormProps> = ({
  ...
  return (
    <Box sx={{ p: 3, border: '1px solid #e0e0e0', borderRadius: '8px', mt: 3 }}>
      <IntlMessages id="common.staffDetails" defaultMessage="รายละเอียดบุคลากร" />
...
const Hr202 = () => {
  ...
  const allStaffsData: StaffData[] = [
    ...
    },
    {
      ...
      first_name_th: 'สีดา', last_name_th: 'สีใจ', middle_name_th: '', first_name_en: 'Suthep',
      last_name_en: 'Inprom', middle_name_en: '', gender: 'หญิง', ethnicity: 'ไทย', nationality: 'ไทย', religion: 'พุทธ', date_of_birth: '1982-12-10', birth_province: 'นครราชสีมา',
    ...
    {
      staff_id: 1009, citizen_id: '1458555525584', foreigner_id: '101', prefixname_id: 1,
      academic_title: 'test', first_name_th: 'กฤกนก', last_name_th: 'กกกนิส', middle_name_th: '', first_name_en: 'Suthep', last_name_en: 'Inprom', middle_name_en: '',
      gender: 'ชาย', ethnicity: 'ไทย', nationality: 'ไทย', religion: 'พุทธ', date_of_birth: '1982-12-10', birth_province: 'นครราชสีมา',
    ...
    },
    {
      ...
      first_name_th: 'สมใจ', last_name_th: 'ใสจม', middle_name_th: '', first_name_en: 'Suthep',
      last_name_en: 'Inprom', middle_name_en: '',
  ...
  const handleStaffSelect = (event: React.SyntheticEvent, value: StaffAutoCompleteOption | null) => {
    ...
    if (value) {
      const staffInfo = allStaffsData.find(staff => staff.staff_id === value.staff_id);
      if (staffInfo) {
        // Dispatch action to set all form data
        dispatch({
          type: 'SET_FORM_DATA',
          payload: {
            ...staffInfo,
            date_of_birth: formatDate(staffInfo.date_of_birth),
            enlistment_date: formatDate(staffInfo.enlistment_date),
            ordained_date: formatDate(staffInfo.ordained_date),
            create_at: formatDate(staffInfo.create_at),
            update_at: formatDate(staffInfo.update_at),
          }
        });
      }
    } else {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | { name?: string; value: unknown }>) => {
    ...
    if (name) { // ตรวจสอบว่า name ไม่ใช่ undefined
