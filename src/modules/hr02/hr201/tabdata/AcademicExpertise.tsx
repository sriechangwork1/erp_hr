...
import { MdSchool, MdPersonOutline, MdClass, MdBook } from "react-icons/md"; // ไอคอนสำหรับความเชี่ยวชาญทางวิชาการ
import VisibilityIcon from '@mui/icons-material/Visibility';

// Data Interface for Academic Expertise Data
...
const staffDetailForTableDisplay = [
  ...
  { staff_id: 601, prefixname_id: 'นาย', first_name_th: 'ผู้บันทึก', last_name_th: 'หนึ่ง' },
  { staff_id: 602, prefixname_id: 'นาง', first_name_th: 'ผู้บันทึก', last_name_th: 'สอง' },
  { staff_id: 603, prefixname_id: 'นางสาว', first_name_th: 'ผู้บันทึก', last_name_th: 'สาม' },
];

// Mock Data for Academic Expertise Information
...
type TableItemProps = {
  ...
  onView: (data: AcademicExpertiseData) => void;
};
  ...
  return (
    <TableRow hover key={data.expertise_id} className="item-hover">
      ...
      <TableCellWrapper align="center">
        {isLoading ? <Skeleton width={80} /> : data.subject_code} <MdBook />
      </TableCellWrapper>
      <TableCellWrapper align="left">
        {isLoading ? <Skeleton width={120} /> : data.teaching_level} <MdClass />
      </TableCellWrapper>
      <TableCellWrapper align="center">
...
const DataTable = ({ data, onView, selectedStaffId }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = useState<keyof AcademicExpertiseData>('expertise_id');
  const [searchQuery, setSearchQuery] = useState<string>('');

  const intl = useIntl();
  const labeltext = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr02.09' }); // สมมติว่ามี ID สำหรับเมนูนี้ (อ้างอิงจาก Academic Expertise)
    return label;
  }, [intl]);
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'subject_code', label: 'รหัสวิชา', minWidth: 80, sortable: true },
      { id: 'teaching_level', label: 'ระดับการสอน', minWidth: 120, sortable: true },
  ...
  const handleRequestSort = (
    ...
    property: keyof AcademicExpertiseData,
  ) => {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      currentFilteredData = currentFilteredData.filter((row) => {
        // Search in expertise_id, expertise_name, isced_code, subject_code, teaching_level, officer_id
        const rowMatches =
          ...
          (row.subject_code && String(row.subject_code).toLowerCase().includes(lowerCaseQuery)) ||
          (row.teaching_level && String(row.teaching_level).toLowerCase().includes(lowerCaseQuery)) ||
                ...
                >
                  {column.sortable ? (
                    <TableSortLabel
                      ...
                      onClick={(event) => handleRequestSort(event, column.id as keyof AcademicExpertiseData)}
                    >
                      {column.label}
                      {orderBy === column.id ? (
            ...
            {filteredRows.length === 0 && (
              <TableRow>
                <TableCell colSpan={columns.length} align="center">
                  ไม่พบข้อมูลความเชี่ยวชาญทางวิชาการสำหรับบุคลากรคนนี้
                </TableCell>
              </TableRow>
            )}
          </TableBody>
...
// Main AcademicExpertise Component
interface AcademicExpertiseProps {
  selectedStaffIdProp: number | null;
}

const AcademicExpertise: React.FC<AcademicExpertiseProps> = ({ selectedStaffIdProp }) => {
  const [currentViewTask, setCurrentViewTask] = useState<AcademicExpertiseData | null>(null);
  const [openViewTask, setOpenViewTask] = useState<boolean>(false);

  const onViewTask = (data: AcademicExpertiseData) => {
  ...
  return (
    <AppCard>
      ...
      <AppDialog
        ...
        title={<IntlMessages id="common.academicExpertiseDetail" defaultMessage="รายละเอียดข้อมูลความเชี่ยวชาญทางวิชาการ" />}
      >
        <Box sx={{ width: '100%', padding: '20px' }}>
          ...
          <TextField
            ...
            margin="normal"
            size="small"
            value={currentViewTask?.subject_code || ''}
            name="subject_code"
            disabled
          />
          <TextField
            ...
            margin="normal"
            size="small"
            value={currentViewTask?.teaching_level || ''}
            name="teaching_level"
            disabled
          />
          <TextField
