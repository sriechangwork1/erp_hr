...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof Data>('name_change_id');
  const [searchQuery, setSearchQuery] = React.useState<string>('');

  const intl = useIntl();

  const labeltext = React.useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr02.05' }); // Assuming new ID
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'name_change_id', label: 'รหัสประวัติ', minWidth: 80, sortable: true },
      { id: 'staff_id', label: 'รหัสบุคลากร', minWidth: 100, sortable: true },
      { id: 'previous_name', label: 'ชื่อ-สกุล (เดิม)', minWidth: 200, sortable: true },
      { id: 'change_date', label: 'วันที่เปลี่ยน', minWidth: 120, sortable: true },
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  key={row.name_change_id}
                  data={row}
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.name_change_id)}
                />
              ))}
          </TableBody>
        </Table>
