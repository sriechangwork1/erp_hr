...
import { StaffDataRaw, StaffDetailByType, StaffTypeSummaryRow, StaffTypeMapping } from './types';
...
// --- Simulated Raw Staff Data Generation (adapted for new STAFFTYPE_ID) ---
const generateSimulatedStaffData = (count: number): StaffDataRaw[] => {
  ...
  const adminPositions = ["นักวิชาการศึกษา", "เจ้าหน้าที่บริหารงานทั่วไป", "นักวิเคราะห์นโยบายและแผน", "เจ้าหน้าที่ธุรการ", "ผู้อำนวยการกอง", "พนักงานขับรถ", "แม่บ้าน"];
  ...
  for (let i = 1; i <= count; i++) {
    ...
    if (staffTypeId === 1 || staffTypeId === 3) { // ข้าราชการ, พนักงานมหาวิทยาลัย (มักมีตำแหน่งวิชาการ)
      if (Math.random() > 0.4) { // More likely to be academic for these types
        positionAcademic = getRandomItem(academicPositions);
      } else {
        positionAdmin = getRandomItem(adminPositions);
      }
    } else { // ลูกจ้างประจำ, พนักงานราชการ, ลูกจ้างชั่วคราว (มักเป็นสายสนับสนุน)
      positionAdmin = getRandomItem(adminPositions);
    }
    ...
    staffData.push({
      ...
      academic_title: positionAcademic ? getRandomItem(["", "ผศ.", "รศ.", "ศ."]) : null,
...
const getPositionName = (staff: StaffDataRaw): string => {
  if (staff.academic_title && staff.position_academic_name_th) return `${staff.academic_title}${staff.position_academic_name_th}`;
...
const Rp106Page = () => {
  ...
  const allFilteredStaff = useMemo(() => {
    ...
    if (filterStaffType) {
      currentData = currentData.filter(staff =>
        staff.staffTypeName === filterStaffType
      );
    }
    ...
    return currentData.sort((a, b) => {
      // Sort by Staff Type first, then by Faculty, then by Full Name
      const typeOrder = Object.values(StaffTypeMapping).indexOf(a.staffTypeName) - Object.values(StaffTypeMapping).indexOf(b.staffTypeName);
  ...
  }, [processedStaffData, filterStaffType, filterFacultyName, filterIsActive]);
  ...
  const staffTypeSummary: StaffTypeSummaryRow[] = useMemo(() => {
    const summaryMap = new Map<string, { total: number; male: number; female: number }>();
    ...
    allFilteredStaff.forEach(staff => {
      ...
      if (summaryMap.has(typeName)) { // Only count if type is in our mapping
    ...
    const summaryRows: StaffTypeSummaryRow[] = Array.from(summaryMap.entries()).map(([typeName, stats]) => ({
      staffTypeName: typeName,
      totalStaff: stats.total,
      maleCount: stats.male,
      femaleCount: stats.female,
    })).sort((a, b) => { // Ensure summary rows are sorted by their defined order
      return Object.values(StaffTypeMapping).indexOf(a.staffTypeName) - Object.values(StaffTypeMapping).indexOf(b.staffTypeName);
    });
  ...
  const staffTypeOptions = useMemo(() => {
    return Object.values(StaffTypeMapping).map(type => ({ value: type, label: type }));
  }, []);
  ...
  const handleExportAllToPdf = useCallback(() => {
    ...
    // --- Report Header ---
    doc.setFontSize(14);
    doc.text('รายงานรายชื่อบุคลากรแยกตามประเภทบุคลากร', margin, currentY);
    currentY += 7;
    doc.setFontSize(12);
    ...
    const summaryRowsForPdf = staffTypeSummary.map(row => ({
    ...
    (doc as any).autoTable({
      ...
      columnStyles: {
        ...
        1: { cellWidth: 25 },  // ประเภทบุคลากร
        2: { cellWidth: 35 },  // ชื่อ-นามสกุล
        3: { cellWidth: 30 },  // ตำแหน่ง
        4: { cellWidth: 45 },  // หน่วยงาน
        5: { cellWidth: 10 },  // เพศ
        6: { cellWidth: 20 },  // วันที่บรรจุ
        // 7: { cellWidth: 20 },  // เบอร์โทร
        // 8: { cellWidth: 30 },  // อีเมล
      },
      didDrawPage: (data: any) => {
        doc.setFontSize(8);
    ...
    doc.save('รายงานรายชื่อบุคลากรตามประเภทบุคลากร.pdf');
    Swal.fire('สำเร็จ!', 'ส่งออก PDF เรียบร้อยแล้ว', 'success');
  }, [allFilteredStaff, staffTypeSummary]);

  const handleExportAllToExcel = useCallback(() => {
    // Prepare Summary data
    const summaryDataForExcel = staffTypeSummary.map(row => ({
    ...
    XLSX.utils.book_append_sheet(workbook, summarySheet, 'สรุปบุคลากรตามประเภท');
    ...
    XLSX.writeFile(workbook, 'รายงานรายชื่อบุคลากรตามประเภทบุคลากร.xlsx');
    Swal.fire('สำเร็จ!', 'ส่งออก Excel เรียบร้อยแล้ว', 'success');
  }, [allFilteredStaff, staffTypeSummary]);

  return (
    <AppsContent
      title= {<IntlMessages id="sidebar.rp01.06" />}
      sx={{
        mb: 2,
        mt: 2,
        py: 0,
    ...
    >
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {<IntlMessages id="sidebar.rp01.06" />}
        </Typography>
