...
const NewCustomers = (props: Props) => {
  ...
  return (
    <AppCard title={messages['eCommerce.newCustomers'] as string} contentStyle={{ px: 0 }}>
      <AppScrollbar sx={{ maxHeight: 300 }}>
        <AppList data={props.newCustomers} renderRow={(item) => <CustomerItem key={item.id} item={item} />} />
      </AppScrollbar>
    </AppCard>
  );
};
