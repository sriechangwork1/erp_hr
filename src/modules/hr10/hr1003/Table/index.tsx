...
import {
  StaffDetailByType,
  StaffTypeSummaryRow,
  StaffJobCategorySummaryRow, // เพิ่มเข้ามา
  Column,
} from '../types';
...
interface TableProps {
  ...
  jobCategorySummary: StaffJobCategorySummaryRow[]; // ข้อมูลสรุปตามสายงาน (เพิ่มเข้ามา)
...
// --- คอลัมน์สำหรับตารางสรุปตามสายงาน (เพิ่มเข้ามา) ---
const jobCategorySummaryTableColumns: readonly Column<StaffJobCategorySummaryRow>[] = [
  { id: 'jobCategoryName', label: 'สายงาน', minWidth: 200 },
  { id: 'totalStaff', label: 'รวม (คน)', minWidth: 80, align: 'right' },
  { id: 'maleCount', label: 'ชาย', minWidth: 50, align: 'right' },
  { id: 'femaleCount', label: 'หญิง', minWidth: 50, align: 'right' },
];
...
const staffColumns: readonly Column<StaffDetailByType | { 'ลำดับ': number }>[] = [
  ...
  { id: 'jobCategoryName', label: 'สายงาน', minWidth: 120 }, // เพิ่มเข้ามา
  { id: 'fullNameTh', label: 'ชื่อ-นามสกุล', minWidth: 180 }, // ปรับลดขนาด
  { id: 'positionName', label: 'ตำแหน่ง', minWidth: 150 },
  { id: 'facultyName', label: 'หน่วยงาน', minWidth: 200 }, // ปรับลดขนาด
...
const Table: React.FC<TableProps> = ({
  ...
  totalFilteredCount,
  staffTypeSummary,
  jobCategorySummary, // รับ prop jobCategorySummary
  ...
  return (
    <Box>
                ...
                {staffTypeSummary.map((row) => (
                  <TableRow key={row.staffTypeName}>
                    <TableCell component="th" scope="row">
                      <Typography
                        fontWeight={row.staffTypeName === 'รวมทั้งหมด' ? 'bold' : 'normal'}
                      >
                        {row.staffTypeName}
                      </Typography>
                    </TableCell>
      ...
      {/* New Summary Section by Job Category */}
      <Box sx={{ mb: 4 }}>
        <Typography variant="h6" gutterBottom>
          2. สรุปจำนวนบุคลากรแยกตามสายงาน
        </Typography>
        <AppCard>
          <TableContainer component={Paper}>
            <MuiTable size="small" aria-label="staff summary by job category table">
              <TableHead>
                <TableRow>
                  {jobCategorySummaryTableColumns.map((column) => (
                    <TableCell
                      key={column.id as string}
                      align={column.align as any}
                      style={{ minWidth: column.minWidth }}
              ...
              <TableBody>
                {jobCategorySummary.map((row) => (
                  <TableRow key={row.jobCategoryName}>
                    <TableCell component="th" scope="row">
                      <Typography
                        fontWeight={row.jobCategoryName === 'รวมทั้งหมด' ? 'bold' : 'normal'}
                      >
                        {row.jobCategoryName}
                      </Typography>
                    </TableCell>
