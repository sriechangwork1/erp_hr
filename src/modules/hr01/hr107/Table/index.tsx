...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.07' }); // เปลี่ยนเป็น hr01.07
    const words = label.split("HR107 "); // เปลี่ยนเป็น HR107
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'academic_position_name', label: 'ชื่อ' + labelText, minWidth: 200, sortable: true },
