...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.03' }); // เปลี่ยนเป็น hr01.03
    const words = label.split("HR103 "); // เปลี่ยนเป็น HR103
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'position_type_name', label: 'ชื่อ' + labelText, minWidth: 200, sortable: true }, // เปลี่ยน id และ label
      { id: 'create_at', label: 'วันที่สร้าง', minWidth: 170, align: 'center', sortable: true },
      { id: 'officer_id', label: 'ผู้บันทึก', minWidth: 120, align: 'center', sortable: true }, // เปลี่ยน id และ label
