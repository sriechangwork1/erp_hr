...
// นำเข้า Data interface จากไฟล์หลักของ hr110
import { Data } from '../index'; 

// กำหนดประเภทสำหรับทิศทางการจัดเรียง
type Order = 'asc' | 'desc';
// --- กำหนดประเภทข้อมูลสำหรับแต่ละคอลัมน์ของตาราง Gender ---
interface Column {
...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.10' }); // เปลี่ยน ID
    const words = label.split("HR110 "); // ปรับการตัดคำ
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'gender_name', label: 'ชื่อ' + labelText, minWidth: 200, sortable: true },
