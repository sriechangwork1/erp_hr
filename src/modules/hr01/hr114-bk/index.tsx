...
const Hr01Page = () => { // เปลี่ยนชื่อเป็น CountryPage หรือ Hr115Page เพื่อความชัดเจน
  ...
  const labelText = useMemo(() => {
    ...
    const label = intl.formatMessage({ id: 'sidebar.hr01.14' }); // สมมติว่าใช้ id นี้
    const words = label.split("HR114 "); // ตัวอย่างการตัด HR114 ออก
  ...
  return (
    <AppCard
      ...
      title={<IntlMessages id="sidebar.hr01.14" />}
      action={
        <Button
          variant="outlined"
          color="primary"
