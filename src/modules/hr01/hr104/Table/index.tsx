...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.04' }); // เปลี่ยนเป็น hr01.04
    const words = label.split("HR104 "); // เปลี่ยนเป็น HR104
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'job_title_name', label: 'ชื่อ' + labelText, minWidth: 200, sortable: true },
      { id: 'abbreviation', label: 'ชื่อย่อ' + labelText, minWidth: 100, sortable: true },
      { id: 'work_group', label: 'กลุ่มงาน', minWidth: 100, sortable: true },
