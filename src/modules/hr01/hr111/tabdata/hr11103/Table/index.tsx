...
// นำเข้า Data interface จากไฟล์หลักของ hr113
import { Data } from '../index'; 

// กำหนดประเภทสำหรับทิศทางการจัดเรียง
type Order = 'asc' | 'desc';
// --- กำหนดประเภทข้อมูลสำหรับแต่ละคอลัมน์ของตาราง Province ---
interface Column {
...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.1103' }); // เปลี่ยน ID
    const words = label.split("HR11103 "); // ปรับการตัดคำ
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'province_name', label: 'ชื่อ' + labelText, minWidth: 200, sortable: true },
