...
interface Column {
  id: keyof TrainingData | 'actions'; // เปลี่ยนเป็น keyof TrainingData
  label: string;
  minWidth?: number;
...
type DataTableProps = {
  ...
  setTableData: React.Dispatch<React.SetStateAction<TrainingData[]>>; // เปลี่ยนเป็น TrainingData[]
...
const DataTable = ({ data, setTableData, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof TrainingData>('training_id'); // เปลี่ยนเป็น training_id
  const [searchQuery, setSearchQuery] = React.useState<string>('');

  const intl = useIntl();

  const labeltext = React.useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr08.06' }); // เปลี่ยน id
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'training_id', label: 'รหัสฝึกอบรม', minWidth: 50, sortable: true },
      { id: 'staff_id', label: 'รหัสบุคลากร', minWidth: 80, sortable: true },
      { id: 'training_name', label: 'ชื่อหลักสูตร', minWidth: 150, sortable: true },
      { id: 'training_type', label: 'ประเภทการฝึกอบรม', minWidth: 120, sortable: true },
      { id: 'development_method', label: 'วิธีการพัฒนา', minWidth: 120, sortable: true },
      { id: 'competency', label: 'สมรรถนะ', minWidth: 100, sortable: true },
      { id: 'start_date', label: 'วันที่เริ่มต้น', minWidth: 100, sortable: true },
      { id: 'end_date', label: 'วันที่สิ้นสุด', minWidth: 100, sortable: true },
      { id: 'institution', label: 'สถาบัน', minWidth: 120, sortable: true },
      { id: 'order_number', label: 'เลขที่คำสั่ง', minWidth: 120, sortable: true },
      { id: 'order_date', label: 'วันที่ออกคำสั่ง', minWidth: 120, sortable: true },
  ...
  const handleRequestSort = (
    ...
    property: keyof TrainingData, // เปลี่ยนเป็น keyof TrainingData
  ) => {
                ...
                >
                  {column.sortable ? (
                    <TableSortLabel
                      ...
                      onClick={(event) => handleRequestSort(event, column.id as keyof TrainingData)} // เปลี่ยนเป็น keyof TrainingData
                    >
                      {column.label}
                      {orderBy === column.id ? (
