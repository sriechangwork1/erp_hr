...
interface Column {
  id: keyof RequestSupportPositionData | 'actions'; // เปลี่ยนเป็น keyof RequestSupportPositionData
  label: string;
  minWidth?: number;
...
type DataTableProps = {
  ...
  setTableData: React.Dispatch<React.SetStateAction<RequestSupportPositionData[]>>; // เปลี่ยนเป็น RequestSupportPositionData[]
  onView: (data: RequestSupportPositionData) => void;
  onEdit: (data: RequestSupportPositionData) => void;
...
const DataTable = ({ data, setTableData, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof RequestSupportPositionData>('request_support_id'); // เปลี่ยนเป็น request_support_id
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'request_support_id', label: 'รหัสการขอ', minWidth: 50, sortable: true },
      { id: 'staff_id', label: 'รหัสบุคลากร', minWidth: 80, sortable: true },
      { id: 'support_position_id', label: 'รหัสตำแหน่ง สส.', minWidth: 120, sortable: true },
  ...
  const handleRequestSort = (
    ...
    property: keyof RequestSupportPositionData, // เปลี่ยนเป็น keyof RequestSupportPositionData
  ) => {
        ...
        <Table stickyHeader aria-label="sticky table">
                ...
                >
                  {column.sortable ? (
                    <TableSortLabel
                      ...
                      onClick={(event) => handleRequestSort(event, column.id as keyof RequestSupportPositionData)} // เปลี่ยนเป็น keyof RequestSupportPositionData
                    >
                      {column.label}
                      {orderBy === column.id ? (
          ...
          <TableBody>
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  key={row.request_support_id} // เปลี่ยน key เป็น request_support_id
                  data={row}
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.request_support_id)} // เปลี่ยน onDelete เป็น request_support_id
                />
              ))}
          </TableBody>
        </Table>
