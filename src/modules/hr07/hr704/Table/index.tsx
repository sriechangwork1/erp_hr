...
interface Column {
  id: keyof SupportExpertiseData | 'actions'; // เปลี่ยนเป็น keyof SupportExpertiseData
  label: string;
  minWidth?: number;
...
type DataTableProps = {
  ...
  setTableData: React.Dispatch<React.SetStateAction<SupportExpertiseData[]>>; // เปลี่ยนเป็น SupportExpertiseData[]
  onView: (data: SupportExpertiseData) => void;
  onEdit: (data: SupportExpertiseData) => void;
...
const DataTable = ({ data, setTableData, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof SupportExpertiseData>('expertise_id'); // เปลี่ยนเป็น expertise_id
  const [searchQuery, setSearchQuery] = React.useState<string>('');

  const intl = useIntl();

  const labeltext = React.useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr07.04' }); // เปลี่ยน id
  ...
  const handleRequestSort = (
    ...
    property: keyof SupportExpertiseData, // เปลี่ยนเป็น keyof SupportExpertiseData
  ) => {
                ...
                >
                  {column.sortable ? (
                    <TableSortLabel
                      ...
                      onClick={(event) => handleRequestSort(event, column.id as keyof SupportExpertiseData)} // เปลี่ยนเป็น keyof SupportExpertiseData
                    >
                      {column.label}
                      {orderBy === column.id ? (
