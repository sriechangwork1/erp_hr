...
interface Column {
  id: keyof AcademicExpertiseData | 'actions'; // เปลี่ยนเป็น keyof AcademicExpertiseData
  label: string;
  minWidth?: number;
...
type DataTableProps = {
  ...
  setTableData: React.Dispatch<React.SetStateAction<AcademicExpertiseData[]>>; // เปลี่ยนเป็น AcademicExpertiseData[]
  onView: (data: AcademicExpertiseData) => void;
  onEdit: (data: AcademicExpertiseData) => void;
...
const DataTable = ({ data, setTableData, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof AcademicExpertiseData>('expertise_id'); // เปลี่ยนเป็น expertise_id
  const [searchQuery, setSearchQuery] = React.useState<string>('');

  const intl = useIntl();

  const labeltext = React.useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr07.03' }); // เปลี่ยน id
    const words = label.split("HR703 "); // เปลี่ยน HR110 เป็น HR111
    return words[1] || '';
  }, [intl]);
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      ...
      { id: 'expertise_name', label: 'ชื่อความเชี่ยวชาญ', minWidth: 200, sortable: true },
      { id: 'isced_code', label: 'รหัส ISCED', minWidth: 100, sortable: true },
      { id: 'subject_code', label: 'รหัสวิชา', minWidth: 100, sortable: true },
  ...
  const handleRequestSort = (
    ...
    property: keyof AcademicExpertiseData, // เปลี่ยนเป็น keyof AcademicExpertiseData
  ) => {
        ...
        <Table stickyHeader aria-label="sticky table">
                ...
                >
                  {column.sortable ? (
                    <TableSortLabel
                      ...
                      onClick={(event) => handleRequestSort(event, column.id as keyof AcademicExpertiseData)} // เปลี่ยนเป็น keyof AcademicExpertiseData
                    >
                      {column.label}
                      {orderBy === column.id ? (
          ...
          <TableBody>
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  key={row.expertise_id} // เปลี่ยน key เป็น expertise_id
                  data={row}
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.expertise_id)} // เปลี่ยน onDelete เป็น expertise_id
                />
              ))}
          </TableBody>
        </Table>
