...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof DocumentData>('document_id');
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'document_id', label: 'รหัสเอกสาร', minWidth: 80, sortable: true },
      { id: 'staff_id', label: 'รหัสบุคลากร', minWidth: 100, sortable: true },
      { id: 'document_name', label: 'ชื่อเอกสาร', minWidth: 180, sortable: true },
      { id: 'document_type', label: 'ประเภทเอกสาร', minWidth: 120, sortable: true },
      { id: 'document_path', label: 'ที่อยู่ไฟล์เอกสาร', minWidth: 200, sortable: true },
      { 
        id: 'create_at', 
        label: 'วันที่สร้าง', 
        minWidth: 120, 
            ...
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  ...
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.document_id)}
