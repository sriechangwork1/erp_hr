...
type DataTableProps = {
  ...
  onEdit: (data: WorkPermitData) => void;
...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof WorkPermitData>('work_permit_id');
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'work_permit_id', label: 'รหัสใบอนุญาต', minWidth: 80, sortable: true },
      ...
      { 
        id: 'expiry_date', 
        label: 'สิ้นสุดการอนุญาต', 
        minWidth: 120, 
        sortable: true,
        format: (value: string) => value ? new Date(value).toLocaleDateString('th-TH') : '-'
      },
      { id: 'document_file', label: 'ไฟล์เอกสาร', minWidth: 150, sortable: true },
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  key={row.work_permit_id}
                  data={row}
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.work_permit_id)}
