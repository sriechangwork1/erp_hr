...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof Data>('salary_id');
  const [searchQuery, setSearchQuery] = React.useState<string>('');

  const intl = useIntl();

  const labeltext = React.useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr02.04' }); // Assuming new ID
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      ...
      { id: 'salary_amount', label: 'เงินเดือน', minWidth: 120, sortable: true },
      ...
      { id: 'academic_allowance', label: 'เงินประจำตำแหน่งวิชาการ', minWidth: 180, sortable: true },
      { id: 'management_allowance', label: 'เงินประจำตำแหน่งบริหาร', minWidth: 180, sortable: true },
      { id: 'effective_date', label: 'วันที่เริ่มมีผล', minWidth: 120, sortable: true },
            ...
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  ...
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.salary_id)}
                />
              ))}
          </TableBody>
        </Table>
