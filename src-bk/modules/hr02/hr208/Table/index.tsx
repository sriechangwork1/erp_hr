...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof PassportData>('passport_id');
  ...
  const columns: readonly Column[] = React.useMemo(
    () => [
      { id: 'passport_id', label: 'รหัสหนังสือเดินทาง', minWidth: 80, sortable: true },
            ...
            {stableSort(filteredRows, getComparator(order, orderBy))
              ...
              .map((row) => (
                <TableItem
                  ...
                  onView={() => onView(row)}
                  onEdit={() => onEdit(row)}
                  onDelete={() => onDelete(row.passport_id)}
