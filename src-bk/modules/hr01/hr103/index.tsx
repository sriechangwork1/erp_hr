...
// --- กำหนดประเภทข้อมูลสำหรับแต่ละแถวในตาราง (คงโครงสร้างเดิมของ hr103 และเพิ่ม update_at) ---
export interface Data {
  id: number;
  position_type_name: string;
  create_at?: string;
  update_at?: string; // เพิ่ม update_at ตามข้อมูลจำลอง
  officer_id: string;
  [key: string]: any;
}

// ข้อมูลจำลองเริ่มต้นสำหรับตาราง (ย้ายมาจาก hr103/table/index.tsx)
const initialAllRows: Data[] = [
  {
    id: 1,
    position_type_name: 'วิชาการ',
    create_at: '24 พฤษภาคม 2567', // ใช้ค่าคงที่เพื่อความสอดคล้อง
    update_at: '24 พฤษภาคม 2567',
    officer_id: 'นายเจ้าหน้าที่ บุคคล'
  },
  {
    id: 2,
    position_type_name: 'ผู้บริหาร',
  ...
  {
    id: 3,
    position_type_name: 'วิชาชีพเฉพาะ',
    create_at: '24 พฤษภาคม 2567',
    update_at: '24 พฤษภาคม 2567',
    officer_id: 'นายเจ้าหน้าที่ บุคคล'
  },
  {
    id: 4,
    position_type_name: 'เชี่ยวชาญเฉพาะ',
    create_at: '24 พฤษภาคม 2567',
    update_at: '24 พฤษภาคม 2567',
    officer_id: 'นายเจ้าหน้าที่ บุคคล'
  },
...
const Hr01Page = () => {
  ...
  // ฟังก์ชันสำหรับดึงข้อความ label จาก intl และจัดการกรณีที่ไม่พบ 'HR103 '
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.03' });
    const words = label.split("HR103 ");
    return words.length > 1 ? words[1] : label; // ถ้าไม่มี HR103 ให้ใช้ label ทั้งหมด
  }, [intl]);
  ...
  const validateData = () => {
    ...
    if (!currentData?.position_type_name) {
      newErrors.position_type_name = 'กรุณากรอกข้อมูลชื่อประเภทตำแหน่ง';
    }
    setErrors(newErrors);
  ...
  return (
    <AppCard
      ...
      title={<IntlMessages id="sidebar.hr01.03" />}
      action={
        <Button
          variant="outlined"
          color="primary"
      ...
      >
        <Box>
          ...
          <TextField
            ...
            margin="normal"
            size="small"
            value={currentData?.position_type_name || ''}
            name="position_type_name"
            onChange={handleInputChange}
            disabled={dialogMode === 'view'}
            error={!!errors.position_type_name}
            helperText={errors.position_type_name}
          />
          <TextField
            fullWidth
