...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.08' }); // เปลี่ยนเป็น hr01.08
    const words = label.split("HR108 "); // เปลี่ยนเป็น HR108
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'support_position_name', label: 'ชื่อ' + labelText, minWidth: 200, sortable: true },
