...
// นำเข้า Data interface จากไฟล์หลักของ hr114
import { Data } from '../index'; 

// กำหนดประเภทสำหรับทิศทางการจัดเรียง
type Order = 'asc' | 'desc';
// --- กำหนดประเภทข้อมูลสำหรับแต่ละคอลัมน์ของตาราง Country ---
interface Column {
...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.1104' }); // เปลี่ยน ID
    const words = label.split("HR11104 "); // ปรับการตัดคำ
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'country_name_th', label: 'ชื่อ' + labelText + ' (TH)', minWidth: 200, sortable: true },
      { id: 'country_name_en', label: 'ชื่อ' + labelText + ' (EN)', minWidth: 200, sortable: true },
      { id: 'country_abb_name', label: 'ชื่อ' + labelText + ' (ตัวย่อ)', minWidth: 150, align: 'center', sortable: true },
