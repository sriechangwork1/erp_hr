...
const DataTable = ({ data, onView, onEdit, onDelete }: DataTableProps) => {
  ...
  const labelText = useMemo(() => {
    const label = intl.formatMessage({ id: 'sidebar.hr01.09' }); // เปลี่ยนเป็น hr01.09
    const words = label.split("HR109 "); // เปลี่ยนเป็น HR109
  ...
  const columns: readonly Column[] = useMemo(
    () => [
      ...
      { id: 'admin_position_name', label: 'ชื่อ' + labelText, minWidth: 200, sortable: true },
      { id: 'position_abbreviation', label: 'ชื่อย่อ', minWidth: 150, align: 'center', sortable: true },
      { id: 'position_term', label: 'วาระ', minWidth: 150, align: 'center', sortable: true },
